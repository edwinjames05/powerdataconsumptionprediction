...
def read_data(df_phase):
    ...
    if df_phase == 'raw_data':
        ...
        data_path = os.path.join(repo_path, 'data', 'raw_data', 'Us_Express.csv')
...
def preprocess_raw_data(listdata):
    """
    ...
    #                             columns={'AmbientAirTemp': 'y'})

    # for sub in procesed_df['y']:
    #     if '?' in sub:
    #         procesed_df.loc[procesed_df['y'] == sub, 'y'] = sub.split('?')[1]
    raw_df= pd.DataFrame(data=listdata)
    repo_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    data_path = os.path.join(repo_path, 'data', 'interim', 'processed_df.csv')

    os.makedirs(os.path.dirname(data_path), exist_ok=True)

    raw_df.to_csv(path_or_buf=data_path, index=False, header=True)
